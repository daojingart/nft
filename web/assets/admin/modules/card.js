layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	var defaultOption = {
		...
		request: {
			...
			, titleName: 'goods_name' //标题名称，默认：title
			, imageName: 'qr_img_path' //图片地址，默认：image
			, remarkName: 'goods_transfee' //备注名称，默认：remark
			, timeName: 'goods_price' //时间名称，默认：time
			, operate: 'operate' //时间名称，默认：time
		},
		response: {
	...
	function createCard(elem, linenum, item, no) {
		...
		var card =
			...
			'<div class="project-list-item-body"> <div class="project-list-item-title"> <h2>' + item.title + '</h2> </div>' +
			'<div class="project-list-item-body-flex"> <div class="project-list-item-text layui-text">￥' + item.time + '</div> ' +
			'<div class="project-list-item-desc">' +item.remark + '</div>' +
			'</div>  '+item.operate+'' +
			'</div> ' +
			'</div > </div > '
		return card;
	...
	function initData(tempData, option) {
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			item.operate = dataList[i][option.request.operate];
			data.data.push(item);
		}
		return data;
	}
