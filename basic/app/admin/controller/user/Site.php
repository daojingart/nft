...
class Site extends Controller
{
     ...
     * @Time: 2022/9/3   20:39
     */
    public function index($key = 'site_setting')
    {
    ...
    public function turnadd($member_id)
    {
        ...
        if (request()->isAjax()){
            ...
            $info = (new Member())->where(['phone' => $param['phone']])->find();
            ...
            foreach ($goodss as $item){
                if(!(new MemberGoods())->toDonation($member_info,$info,$item['id'])){
                    continue;
                }
            }
        ...
        $member = Member::where('real_status',2)->field('real_status,member_id,phone,name')->select();
