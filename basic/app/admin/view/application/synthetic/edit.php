                    ...
                    <div class="widget-body">
                        <fieldset>
                            <div class="widget-head am-cf">
                                <div class="widget-title am-fl">编辑合成藏品</div>
                            </div>
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" value="{{$data.name?=$data.name}}" name="synthetic[name]">
                                </div>
                            </div>
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <select name="synthetic[goods_ids][]"  multiple data-am-selected="{searchBox: 1, btnSize: 'sm',  placeholder:'请选择合成的藏品支持多选',maxHeight:'200px'}">
                                        ...
                                        <?php if (isset($goods_list)): foreach ($goods_list as $first): ?>
                                            <option value="<?= $first['goods_id'] ?>" <?= in_array($first['goods_id'], $data['goods_id']) ? 'selected' : '' ?> ><?= $first['goods_name'] ?></option>
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" name="synthetic[count]" value="{{$data.count?=$data.count}}">
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" name="synthetic[chance]" value="{{$data.chance?=$data.chance}}">
                                    <small>合成总概率为10,请填写小于10的数字</small>
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">排序</label>
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" name="synthetic[sort]" value="{{$data.sort?=$data.sort}}">
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">白名单提前时间(分钟)</label>
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" name="synthetic[whitelist_time]" placeholder="请输入提前进场的时间分钟" value="{{$data.whitelist_time}}">
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">选择时间</label>
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" name="synthetic[time]" autocomplete="off" value="{{$data.start_time?=$data.start_time}} ~ {{$data.end_time?=$data.end_time}}" id="test6" >
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">合成介绍</label>
                                <div class="am-u-sm-9 am-u-end">
                                    <textarea id="container" name="synthetic[content]">{{$data.content?=$data.content}}</textarea>
                                </div>
                            </div>
                            <div class="am-form-group">
...
<script>
    ...
    $(document).on("click",".addMember",function(){
        layer.open({
            ...
            offset: 'auto',
            area: ['1020px','670px'],
            content: "{{:url('user.preference/goods')}}?types=yxg"
        });
    })
