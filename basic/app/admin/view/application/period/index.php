        ...
        <div class="layui-row">
            ...
            <div class="layui-card-body">
                <div class="layui-col-md3 bl-layui-left">
                    <a class="layui-btn layui-btn-normal" href="<?= url('application.period/add') ?>">新增往期</a>
                </div>
            </div>
            <!--添加按钮-->
        </div>
...
<script type="text/html" id="image">
    <a href="{{d.image}}" title="点击查看大图" target="_blank">
        <img src="{{d.image}}" alt="封面图">
    </a>
</script>
<!-- 图片替换 -->
...
<script>
    layui.use(['table', 'form', 'jquery'], function() {
        ...
        table.render({
            elem: '#user-table',
            url: "{{:url('application.period/index')}}",
            page: true,
            cols: cols,
            skin: 'line'
        });
        ...
        window.remove = function(obj) {
            layer.confirm('确定要删除吗', {
                ...
                $.getJSON("{{:url('application.period/remove')}}", {'id' : obj.data['id']}, function(result){
                    layer.close(loading);
                    if(1 === result.code){
