        ...
        <div class="layui-row">
            ...
            <div class="layui-card-body">
                <div class="layui-col-md3 bl-layui-left">
                    <a class="layui-btn layui-btn-normal" href="<?= url('collection.goods/add') ?>">新增藏品</a>
                </div>
            </div>
            <!--添加按钮-->
        </div>
...
<script>
    layui.use(['table', 'form', 'jquery','laydate'], function() {
        ...
        let tableIns = table.render({
            elem: '#goods-table',
            url: "{{:url('collection.goods/index')}}",
            page: true,
            cols: cols,
            skin: 'line',
            where: {
        ...
        form.on('checkbox(putShelf)', function(data){
            ...
            layer.confirm(text_tip, {
                ...
                $.getJSON("{{:url('collection.goods/operation')}}", {'goods_id' : goods_id,'goods_status':goods_status}, function(result){
                    layer.close(loading);
                    if(1 === result.code){
        ...
        //售卖状态
        form.on('checkbox(soldStatus)', function(data){
            let goods_id = data.elem.getAttribute('data-goods-id');
            let goods_status = data.value;
            let text_tip = '确认修改成已售罄吗？';
            if(goods_status == '10'){
                text_tip = '确认开放购买吗？';
            }
            layer.confirm(text_tip, {
                ...
                layer.close(index);
                let loading = layer.load();
                $.getJSON("{{:url('collection.goods/soldstatus')}}", {'goods_id' : goods_id,'is_sold':goods_status}, function(result){
                    layer.close(loading);
                    if(1 === result.code){
                        layer.msg(result.msg, {
                            icon: 1,
        ...
        window.remove = function(obj) {
            layer.confirm('确定要删除吗', {
                ...
                layer.close(index);
                let loading = layer.load();
                $.getJSON("{{:url('collection.goods/remove')}}", {'goods_id' : obj.data['goods_id']}, function(result){
                    layer.close(loading);
                    if(1 === result.code){
                        layer.msg(result.msg, {
                            icon: 1,
