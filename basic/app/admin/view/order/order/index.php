    ...
    <div class="layui-card-body">
        <div class="layui-tab layui-tab-brief" lay-filter="shopOrder">
            <ul class="layui-tab-title">
                <li class="layui-this" data-url="{{:url('order.order/index')}}">全部</li>
                <li data-url="{{:url('order.order/payList')}}">已支付</li>
                <li data-url="{{:url('order.order/cancelList')}}">已取消</li>
                <li data-url="{{:url('order.order/refundList')}}">已退款</li>
            </ul>
        </div>
        <!--内容区域-->
...
<script>
    ...
    layui.use(['table', 'form', 'jquery','element','excel','laydate'], function() {
        ...
        $("#table_content").html(template('order_list'));
        request_url = "{{:url('order.order/index')}}";
