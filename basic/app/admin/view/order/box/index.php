    ...
    <div class="layui-card-body">
        <div class="layui-tab layui-tab-brief" lay-filter="shopOrder">
            <ul class="layui-tab-title">
                <li class="layui-this" data-url="{{:url('order.box/index')}}">全部</li>
                <li data-url="{{:url('order.box/payList')}}">已支付</li>
                <li data-url="{{:url('order.box/cancelList')}}">已取消</li>
                <li data-url="{{:url('order.box/refundList')}}">已退款</li>
            </ul>
            <!--内容区域-->
...
<script>
    ...
    layui.use(['table', 'form', 'jquery','element','excel','laydate'], function() {
        ...
        $("#table_content").html(template('order_list'));
        request_url = "{{:url('order.box/index')}}";
        getTableData(request_url,localStorage.getItem("box_order_page_curr"));
    });
    ...
    function getTableData(url,page)
    {
        table.render({
            ...
            page: {
                ...
                localStorage.setItem("box_order_page_curr",curr);//存储页码
            }
        });
        //搜索条件查询操作
