                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    ...
                                    <div class="bl-layui-card layui-form">
                                        <div class="layui-card-header">
                                            <input lay-filter="encrypt" value="{{$vo.id}}" type="checkbox" name="role[rule][]" title="{{$vo.node_name}}" lay-skin="primary" {{if in_array($vo.id,explode(",",$model['rule']))}}  checked {{/if}}>
                                        </div>
                                        {{if isset($vo['submenu'])}}
                                        <div class="layui-card-body layui-row layui-col-space10" style="margin-left: 25px">
                                            <div class="layui-col-md12">
                                                {{volist name="$vo['submenu']" id="v"}}
                                                <input class="checknode{{$vo.id}}" lay-filter="encrypt" value="{{$v.id}}" type="checkbox" name="role[rule][]" title="{{$v.node_name}}" lay-skin="primary"  {{if in_array($v.id,explode(",",$model['rule']))}}  checked {{/if}}>
                                                {{if isset($v['submenu'])}}
                                                <div class="layui-card-body layui-row layui-col-space10" style="margin-left: 25px">
                                                    <div class="layui-col-md12">
                                                        {{volist name="$v['submenu']" id="v1"}}
                                                        <input class="checknode{{$vo.id}} checknode{{$v.id}}" value="{{$v1.id}}" type="checkbox" name="role[rule][]" title="{{$v1.node_name}}" lay-skin="primary"  {{if in_array($v1.id,explode(",",$model['rule']))}}  checked {{/if}}>
                                                        {{/volist}}
                                                    </div>
                                                </div>
                                                {{/if}}
