...
<div class="layui-card">
    <ul class="layui-tab-title store_system">
        ...
        <li class="layui-this"><a href="{{:url('user.site/internal')}}">内测名单</a></li>
    </ul>
    <!--        表格上方的数据-->
...
<script type="text/html" id="invitations_number">
    <div style="display: flex">
        <span style="cursor:pointer;" class="bl-table-name-title bl-Edit-title" data-id="{{d.member_id}}" data-field="z" data-name="{{d.invitations_number}}">
            {{d.invitations_number}} <i class="layui-icon layui-icon-survey" style="font-size: 18px !important;"></i>
        </span>
        <span style="cursor:pointer;" class="bl-table-name-title bl-Edit-title" data-id="{{d.member_id}}" data-field="m" data-name="{{d.moon_invitations_number}}">
            {{d.moon_invitations_number}} <i class="layui-icon layui-icon-survey" style="font-size: 18px !important;"></i>
        </span>
        <span style="cursor:pointer;" class="bl-table-name-title bl-Edit-title" data-id="{{d.member_id}}" data-field="w" data-name="{{d.week_invitations_number}}">
            {{d.week_invitations_number}} <i class="layui-icon layui-icon-survey" style="font-size: 18px !important;"></i>
        </span>
    </div>
...
<script>
    layui.use(['table', 'form', 'jquery','toast','dropdown','laydate','upload','excel'], function() {
        ...
        let tableIns = table.render({
            elem: '#user-table',
            id:'videoID',
            url: "{{:url('user.index/test')}}",
            page: {
                curr:localStorage.getItem("member_page_curr_test_test")?localStorage.getItem("member_page_curr_test"):1,
            },
            cols: cols,
            skin: 'line',
            done: function (res, curr, count) {
                localStorage.setItem("member_page_curr_test",curr);//存储页码
                dropdown.render({
                    elem: '.blMore'
                    ,trigger: 'hover'
                    ,data: [
                    ...
                    if(1 === result.code){
                        layer.msg(result.msg, {
                            ...
                            tableIns.reload({
                                ...
                                page: {
                                    curr:localStorage.getItem("member_page_curr_test")?localStorage.getItem("member_page_curr_test"):1,
                                },
                                cols: cols,
                                skin: 'line',
                                done: function (res, curr, count) {
        ...
        function uploadExcel(files) {
            try {
                excel.importExcel(files, {
                    ...
                    $.ajax({
                        async: false,
                        url: "{{:url('user.index/importOrderTest')}}",
                        type: 'post',
                        dataType: "json",
